    ...
    public class MyTask
    {
            ...
            lock (this)
            {
                ...
                if (_continuation is not null)
                {
                    MyThreadPool.QueueUserWorkItem(delegate
                    {
                        ...
                        {
                            ExecutionContext.Run(_context, (object? state) => ((Action)state!).Invoke(), 
                                _continuation);
                        }
                    });
                }
        ...
        public void Wait()
        {
            ...
            if (_exception is not null)
            {
                ...
                //之所以返回AggregateException而不是Exception，是因为Task中可能包含多个Task,
                //例如Task.WhenAll返回的Task对象
            ...
            void MoveNext()
            {
                try
                {
                    while (e.MoveNext())
                    {
                        ...
                        //如果next表示的Task同步完成了，
                        //就不需要使用ContinueWith方法异步的调用MoveNext方法了
                        if (next.IsCompleted)
                        {
                            next.Wait();
                            continue;
